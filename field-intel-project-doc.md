...
- **Splash screen:** Blue gradient splash with app icon, "Field Intel / Agility Ortho" text, and loading spinner. Fades out once auth check completes (whether auto-login or showing login screen).
- **New tag: Observation** — Added to both the note form and feed filter bar. Tags now: Surgeon Interaction, Case Feedback, Competitive Intel, Product Interest, Issue / Complaint, Observation.
- **Soft-delete notes:** Reps see a ✕ button on their own notes. Tapping it removes the note from their feed. But John (Owner) still sees all deleted notes — they appear faded with a red left border and a "Deleted by [name]" tag. DB columns: `deleted_at`, `deleted_by` (auth UUID), `deleted_by_rep_id` (integer FK→reps).
- **Service worker cache bumped to v2** — Forces stale cache purge on next visit.
- **Auth session persistence confirmed working** — The re-login John saw was expected (V2 switched Supabase projects). Session now persists across visits.
...
- `field_notes` — (rep_id FK→reps, surgeon_id FK→surgeons, subject_text, note_body, tags[], note_date, next_action, photo_url, created_at, deleted_at, deleted_by, deleted_by_rep_id FK→reps)
...
- 2026-04-02 — Session 3: Added splash screen (blue gradient, app icon, loading spinner, fade-out transition). Added "Observation" tag to form and feed filters. Built soft-delete: reps can delete own notes (disappear from their feed), Owner sees all deleted notes with faded styling + "Deleted by [name]" red tag. Added `deleted_at`, `deleted_by`, `deleted_by_rep_id` columns via migration. Bumped service worker cache to v2. Confirmed auth session persistence is working (first re-login was expected due to Supabase project switch).
